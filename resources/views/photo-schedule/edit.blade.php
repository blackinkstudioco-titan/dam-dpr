            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                        <h3 class="font-semibold text-lg mb-3">Informasi Foto</h3>
                        <div class="flex items-start gap-4">
                            <img src="{{ $foto->thumbnail_url }}" 
                                 alt="{{ $foto->judul }}" 
                                 class="h-32 w-32 object-cover rounded-lg shadow">
                            <div class="flex-1">
                                <h4 class="text-xl font-semibold text-gray-900">{{ $foto->judul }}</h4>
                                <p class="text-sm text-gray-500 mt-1">{{ $foto->mm_id }}</p>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($foto->scheduled_publish_at)
                                            <span class="px-2 py-1 bg-green-100 text-green-800 rounded text-xs">
                                                📤 Publish pada {{ $foto->scheduled_publish_at->format('d M Y H:i') }} WIB
                                            </span>
                                        @elseif($foto->scheduled_unpublish_at)
                                            <span class="px-2 py-1 bg-red-100 text-red-800 rounded text-xs">
                                                📥 Unpublish pada {{ $foto->scheduled_unpublish_at->format('d M Y H:i') }} WIB
                                            </span>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <form action="{{ route('photo-schedule.update', $foto->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="action" 
                                ...
                                <option value="publish" {{ ($foto->scheduled_publish_at || old('action') === 'publish') ? 'selected' : '' }}>
                                    📤 Publish (Tampilkan foto)
                                </option>
                                <option value="unpublish" {{ ($foto->scheduled_unpublish_at || old('action') === 'unpublish') ? 'selected' : '' }}>
                            ...
                            <p class="text-gray-500 text-xs mt-1">
                                <strong>Publish:</strong> Foto akan ditampilkan di galeri publik<br>
                                <strong>Unpublish:</strong> Foto akan disembunyikan dari galeri publik
                            </p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" 
                                   name="scheduled_at" 
                                   id="scheduled_at" 
                                   value="{{ old('scheduled_at', ($foto->scheduled_publish_at ?? $foto->scheduled_unpublish_at)?->format('Y-m-d\TH:i')) }}"
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <div class="flex gap-2 px-4 py-3">
                    ...
                    <form action="{{ route('photo-schedule.cancel', $foto->id) }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit"
