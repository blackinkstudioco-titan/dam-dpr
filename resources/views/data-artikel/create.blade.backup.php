<x-app-layout>
  ...
  function getItemLabel(item) {
    ...
    // Coba beberapa nama properti yang lazim
  ...
  async function searchAnggota(term) {
    ...
    try {
      const response = await fetch(`/api/anggota-dpr/search?term=${encodeURIComponent(term)}`, {
      ...
      // Pastikan array, lalu normalisasi ke label string unik, buang kosong
      ...
      if (labels.length > 0) {
        ...
        anggotaList.innerHTML = '<div class="px-4 py-2 text-sm text-gray-500">Tidak ada suggestion</div>';
        anggotaBox.classList.remove('hidden');
      }
    } catch (err) {
      loadingAnggota.classList.add('hidden');
  ...
  function insertAnggota(value) {
    ...
    let beforeWord;
    if (lastComma === -1) {
      beforeWord = ''; // tidak ada koma sebelumnya
    } else {
      beforeWord = before.substring(0, lastComma + 1); // termasuk koma
    }
    ...
    anggotaInput.value = anggotaInput.value
      .replace(/\s*,\s*/g, ', ')   // konsisten spasi setelah koma
      .replace(/,\s*,/g, ', ')     // buang koma ganda
      .replace(/\s{2,}/g, ' ')     // buang spasi beruntun
      .replace(/^,\s*/,'')         // buang koma di awal
      .replace(/,\s*$/,'');        // buang koma di akhir
  ...
  anggotaInput.addEventListener('input', () => {
    ...
    anggotaTimeout = setTimeout(() => searchAnggota(getCurrentAnggotaWord()), 300);
  });
