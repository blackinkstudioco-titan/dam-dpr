            ...
            @forelse ($artikels as $i => $a)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="border p-2 text-center">
                        ...
                        <a href="{{ route('artikel.edit', $a->id) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a> |
                        <form action="{{ route('artikel.destroy', $a->id) }}" method="POST" class="inline">
                            @csrf @method('DELETE')
                            <button onclick="return confirm('Hapus artikel ini?')" class="text-red-500">Hapus</button>
                        </form>
                    </td>
                </tr>
            @empty
